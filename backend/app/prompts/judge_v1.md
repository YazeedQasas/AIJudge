...
description: >
  Initial grounded Arabic judge prompt, extracted verbatim from the original
  hardcoded build_prompt(). Instructs the model to answer only from the numbered
  sources, cite with [n], refuse when sources are insufficient, and answer in Arabic.
changelog:
